...
namespace View
{
    ...
    public partial class SearchDiscountForm : Form
    {
        ...
        public SearchDiscountForm(BindingList<IDiscount> discountList)
        {
            ...
            string[] ShopComboBoxItems = new string[0];
            int arrayIndex = 0;
            foreach (IDiscount discount in _listDiscountSearch)
            {
                if (discount is DiscountBase discountBase)
                {
                    Array.Resize(ref ShopComboBoxItems, ShopComboBoxItems.Length + 1);
                    ShopComboBoxItems[arrayIndex] = discountBase.Shop;
                    arrayIndex++;
                }
            }
            ShopComboBox.Items.AddRange(ShopComboBoxItems.Distinct().ToArray());
        }
